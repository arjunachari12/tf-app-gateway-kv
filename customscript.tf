...
resource "azurerm_storage_account" "arjunsfsfga434a" {
  name                     = "arjunsfsfga434a"
...
resource "azurerm_storage_container" "data" {
  name                  = "data"
  storage_account_name  = "arjunsfsfga434a"
  container_access_type = "blob"
  depends_on = [
    azurerm_storage_account.arjunsfsfga434a
  ]
}
...
resource "azurerm_virtual_machine_extension" "vmextension" {
  ...
  settings = <<SETTINGS
    {
        "fileUris": ["https://${azurerm_storage_account.arjunsfsfga434a.name}.blob.core.windows.net/data/IIS_Config_${each.key}.ps1"],
